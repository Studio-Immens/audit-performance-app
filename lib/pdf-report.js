...
try {
  ...
  console.error('[PDF] Cannot create reports dir:', e.message);
}
...
async function generateReport(auditData) {
  return new Promise((resolve, reject) => {
    ...
    // SEO Score
    y = sectionTitle('SEO & GEO (AI Optimization)', y + 6);
    const seoScore = auditData.seoScore || 0;
    const geo = auditData.geoScore || {};
    const geoChecks = geo.checks || [];
    y = tableRow(y, 'Performance (Google PSI)', (auditData.score || 0) + '/100', scoreColor(auditData.score || 0));
    y = tableRow(y, 'SEO Score', (seoScore || 'N/D'), seoScore >= 80 ? '#4ade80' : seoScore >= 50 ? '#fbbf24' : '#6a6a7a');
    y = tableRow(y, 'GEO Score (AI Readiness)', (geo.score || 0) + '/' + (geo.maxScore || 100), (geo.score || 0) >= 60 ? '#4ade80' : '#fbbf24');
    y = tableRow(y, 'GEO Label', geo.label || 'N/D', textClr);

    // GEO checklist
    if (geoChecks.length > 0) {
      y += 4;
      geoChecks.forEach(function(check) {
        if (y > pageH - 60) {
          footer();
          doc.addPage();
          y = doc.page.margins.top + 20;
        }
        doc.rect(doc.page.margins.left + 10, y, pageW - 20, 20).fill(cardBg);
        doc.fontSize(7).fillColor(check.passed ? '#4ade80' : '#f87171').font('Helvetica-Bold');
        doc.text(check.passed ? '✅' : '❌', doc.page.margins.left + 16, y + 4, { width: 14 });
        doc.fontSize(7).fillColor(textClr).font('Helvetica');
        doc.text(check.name, doc.page.margins.left + 32, y + 4, { width: pageW - 100 });
        doc.fontSize(6).fillColor(textMut).font('Helvetica');
        doc.text(check.detail || '', doc.page.margins.left + 32, y + 13, { width: pageW - 100 });
        y += 22;
      });
    }
